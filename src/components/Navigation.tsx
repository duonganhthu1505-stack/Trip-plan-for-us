...
export const Navigation: React.FC<NavigationProps> = ({
  ...
  const mainTabs = [
    ...
    { id: 'settings' as ActiveTab, label: lang === 'vi' ? 'Cài đặt & phân quyền' : 'Settings & Access', icon: SettingsIcon }
  ];
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16 sm:h-18 gap-2">
          <div className="flex items-center gap-3 shrink-0">
            <button onClick={() => onSelectTab('overview')} className="flex items-center gap-2.5 text-left group cursor-pointer">
              <img src="/pwa-192x192.png" alt="Our Travel Planner Icon" className="w-10 h-10 rounded-xl object-cover border border-[#D5A85A]/70 shadow-xs transition-transform group-hover:scale-105" />
              <div className="hidden sm:block">
                <span className="block font-serif text-lg font-bold text-[#382D24] leading-tight group-hover:text-[#5C4033] transition-colors">Our Travel Planner</span>
          ...
          <div className="relative flex-1 min-w-0 max-w-[140px] xs:max-w-[180px] sm:max-w-xs md:max-w-sm">
            <button id="nav-trip-selector-btn" type="button" onClick={() => setTripDropdownOpen(!tripDropdownOpen)} className="flex items-center justify-between gap-1.5 sm:gap-2 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl bg-[#FAF7F2] hover:bg-[#F3ECE2] border border-[#E2D4C3] text-xs sm:text-sm text-[#382D24] transition-all w-full text-left cursor-pointer shadow-2xs">
              <div className="truncate flex items-center gap-1.5">
                <span className="text-[#8C6D58] hidden md:inline font-medium">{lang === 'vi' ? 'Chuyến đi:' : 'Our Trips:'}</span>
                <span className="font-serif font-bold text-[#382D24] truncate">{currentTrip ? currentTrip.name : (lang === 'vi' ? 'Chưa chọn' : 'No trip')}</span>
              </div>
              <ChevronDown className={`w-3.5 h-3.5 sm:w-4 sm:h-4 text-[#8C6D58] shrink-0 transition-transform ${tripDropdownOpen ? 'rotate-180' : ''}`} />
            </button>
            {tripDropdownOpen && (
              <div id="nav-trip-dropdown" className="absolute left-0 sm:left-auto sm:right-0 mt-2 w-72 sm:w-80 bg-[#FFFDF9] border border-[#E2D4C3] rounded-2xl shadow-xl py-2 z-50 animate-in fade-in">
                <div className="px-3.5 py-2 border-b border-[#F0E6D8] flex items-center justify-between">
                  <span className="text-xs font-semibold uppercase tracking-wider text-[#8C6D58]">{lang === 'vi' ? 'Đổi chuyến đi' : 'Switch Journey'}</span>
                  <button id="nav-dropdown-new-trip-btn" onClick={() => { setTripDropdownOpen(false); onNewTrip(); }} className="flex items-center gap-1 text-xs font-medium text-[#6E4F36] hover:text-[#382D24] bg-[#EFE6DB] hover:bg-[#E5DACB] px-2.5 py-1 rounded-lg transition-colors cursor-pointer">
                    <Plus className="w-3.5 h-3.5" /><span>{lang === 'vi' ? 'Chuyến mới' : 'New Trip'}</span>
                  </button>
                </div>
                <div className="max-h-64 overflow-y-auto py-1 divide-y divide-[#F6EFE6]">
                  {allTrips.map((t) => {
                    const isSelected = currentTrip?.id === t.id;
                    const statusText = t.status === 'Ongoing' ? (lang === 'vi' ? 'Đang diễn ra' : 'Ongoing') : t.status === 'Upcoming' ? (lang === 'vi' ? 'Sắp tới' : 'Upcoming') : t.status === 'Completed' ? (lang === 'vi' ? 'Đã xong' : 'Completed') : (lang === 'vi' ? 'Lên ý tưởng' : 'Planning');
                    return (
                      <button key={t.id} id={`trip-option-${t.id}`} onClick={() => { onSelectTrip(t.id); setTripDropdownOpen(false); }} className={`w-full text-left px-3.5 py-2.5 flex items-start gap-2.5 transition-colors cursor-pointer ${isSelected ? 'bg-[#FAF7F2]' : 'hover:bg-[#FAF7F2]'}`}>
                        <div className={`w-2 h-2 rounded-full mt-2 shrink-0 ${isSelected ? 'bg-[#6E4F36]' : 'bg-[#D9CABB]'}`} />
                        <div className="min-w-0 flex-1">
                          <p className="font-serif text-sm font-semibold text-[#382D24] truncate">{t.name}</p>
                          <p className="text-xs text-[#8C6D58] truncate">{t.destination} {t.startDate ? `• ${t.startDate}` : ''}</p>
                        </div>
                        <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium shrink-0 ${t.status === 'Ongoing' ? 'bg-[#E3EFE5] text-[#2F6636]' : t.status === 'Upcoming' ? 'bg-[#EBF2F8] text-[#2A527A]' : t.status === 'Completed' ? 'bg-[#EFE8DE] text-[#6E4F36]' : 'bg-[#F6EBE1] text-[#915B35]'}`}>{statusText}</span>
                      </button>
                    );
                  })}
                </div>
          ...
          <div className="flex items-center gap-1.5 sm:gap-2 shrink-0">
            {!isConnectedToCloud ? (
              ...
              <button id="nav-sync-indicator-btn" type="button" onClick={onForceCloudSync} className="hidden sm:flex items-center gap-1.5 px-2.5 py-1.5 rounded-xl bg-[#FAF7F2] hover:bg-[#F3ECE2] active:scale-[0.98] border border-[#E8DEC8] text-xs transition-all cursor-pointer group shadow-2xs">
                {syncStatus === 'syncing' ? <><RefreshCw className="w-3.5 h-3.5 text-[#B07D62] animate-spin" /><span className="text-[#8C6D58] font-medium text-[11px]">Đang đồng bộ...</span></> : syncStatus === 'offline' ? <><div className="w-2 h-2 rounded-full bg-[#A68972]" /><span className="text-[#8C6D58] font-medium text-[11px]">Thử kết nối lại</span></> : <><div className="w-2 h-2 rounded-full bg-[#34A853] animate-pulse" /><Cloud className="w-3.5 h-3.5 text-[#34A853]" /><span className="text-[#382D24] font-medium text-[11px] hidden lg:inline">Đồng bộ ĐT & Web</span><RefreshCw className="w-3 h-3 text-[#8C6D58] group-hover:rotate-180 transition-transform" /></>}
              </button>
            )}
            <button id="nav-save-trip-btn" onClick={onManualSave} className="flex items-center gap-1.5 px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-xl bg-[#5C4033] hover:bg-[#483226] text-white text-xs sm:text-sm font-medium shadow-xs transition-colors cursor-pointer shrink-0" title={lang === 'vi' ? 'Lưu chuyến đi' : 'Save Trip'}>
              <Save className="w-3.5 h-3.5 sm:w-4 sm:h-4" /><span className="hidden sm:inline">{lang === 'vi' ? 'Lưu' : 'Save'}</span>
            </button>
            ...
            <button id="nav-theme-btn" onClick={() => setIsDarkMode((value) => !value)} className="hidden sm:flex p-2 rounded-xl border bg-[#FAF7F2] border-[#E8DEC8] text-[#6E4F36] hover:bg-[#F3ECE2] transition-colors cursor-pointer" title={isDarkMode ? 'Chuyển sang chế độ sáng' : 'Chuyển sang chế độ tối'} aria-label={isDarkMode ? 'Chế độ sáng' : 'Chế độ tối'}>
            ...
            <button id="nav-mobile-menu-btn" onClick={() => setMobileMenuOpen(!mobileMenuOpen)} className="md:hidden p-2 rounded-xl bg-[#FAF7F2] active:bg-[#EFE6DB] border border-[#E8DEC8] text-[#4A2F22] hover:text-[#2E1A11] transition-colors cursor-pointer shadow-2xs flex items-center justify-center shrink-0" title="Menu danh mục" aria-label="Menu danh mục">
        ...
        <div className="hidden md:flex items-center space-x-1 sm:space-x-1.5 py-1.5 sm:py-2 border-t border-[#F2ECE1] overflow-x-auto scrollbar-none">
          {mainTabs.map((tab) => {
            ...
            return <button key={tab.id} id={`nav-tab-${tab.id}`} onClick={() => onSelectTab(tab.id)} className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-3.5 py-1.5 sm:py-2 rounded-xl text-xs sm:text-sm font-medium transition-all whitespace-nowrap cursor-pointer shrink-0 ${isActive ? 'bg-[#5C4033] text-[#FFFDF9] shadow-2xs font-semibold' : 'text-[#6E4F36] hover:bg-[#F3ECE2] hover:text-[#382D24] bg-[#FAF7F2]/60'}`}><Icon className={`w-3.5 h-3.5 sm:w-4 sm:h-4 ${isActive ? 'text-[#FAF7F2]' : 'text-[#8C6D58]'}`} /><span>{tab.label}</span></button>;
          })}
        </div>
      </div>

      {mobileMenuOpen && (
        <div id="nav-mobile-menu" className="md:hidden bg-[#FFFDF9] border-t border-[#E8DEC8] px-4 py-3.5 space-y-3 animate-in slide-in-from-top-2 shadow-xl max-h-[calc(100vh-4.5rem)] overflow-y-auto">
          <div className="flex items-center justify-between px-1 pb-2 border-b border-[#F0E6D8]">
            <span className="text-xs font-semibold text-[#8C6D58] uppercase tracking-wider">{lang === 'vi' ? 'Danh mục màn hình' : 'Navigation Menu'}</span>
          ...
          <div className="pt-3 border-t border-[#E8DEC8] grid grid-cols-3 gap-2">
            ...
            <button onClick={() => setIsDarkMode((value) => !value)} className="flex items-center justify-center gap-2 py-2 px-3 rounded-xl border border-[#E8DEC8] bg-[#FAF7F2] text-xs font-semibold text-[#6E4F36] active:bg-[#EFE6DB] cursor-pointer">{isDarkMode ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}<span>{isDarkMode ? 'Sáng' : 'Tối'}</span></button>
