...
import { getSharedToken, loginSharedPassword, logoutSharedSession, fetchSharedState, saveSharedState, changeSharedPassword, clearSharedSession } from './utils/sharedSync';
import {
...
export default function App() {
  ...
  const [userEmail, setUserEmailState] = useState<string | null>(() => getSharedToken() ? 'Hai đứa' : null);
  const [firebaseUser, setFirebaseUser] = useState<User | null>(null);
  const [sharedSession, setSharedSession] = useState<boolean>(() => Boolean(getSharedToken()));
  ...
  const lastSharedFingerprintRef = useRef('');
  ...
  // Shared password session. Firebase Auth is intentionally not used for access or sync.
  useEffect(() => {
    ...
    const hydrate = async () => {
      ...
      try {
        ...
        if (remote?.data) {
          ...
          const nextRevision = await saveSharedState(local, 0);
          sharedRevisionRef.current = nextRevision;
          lastSharedFingerprintRef.current = JSON.stringify(local);
        }
        sharedReadyRef.current = true;
        setSyncStatus('synced');
      } catch (error) {
        console.warn('Shared sync hydrate failed:', error);
        if (mounted) {
          setSyncStatus('offline');
          showToast('Chưa kết nối được kho dữ liệu chung.', 'error');
        }
      }
    };
    hydrate();
    return () => { mounted = false; };
  }, [sharedSession, showToast]);

  // Full AppData is stored in the protected shared store. LocalStorage remains
  // the offline cache, while this effect pushes edits and polls the other phone.
  useEffect(() => {
    if (!sharedSession || !sharedReadyRef.current) return;
    const fingerprintNow = JSON.stringify(appData);
    if (sharedHydratingRef.current || fingerprintNow === lastSharedFingerprintRef.current) return;
    if (sharedSaveTimerRef.current) window.clearTimeout(sharedSaveTimerRef.current);
    sharedSaveTimerRef.current = window.setTimeout(async () => {
      setSyncStatus('syncing');
      try {
        const nextRevision = await saveSharedState(appDataRef.current, sharedRevisionRef.current);
        sharedRevisionRef.current = nextRevision;
        lastSharedFingerprintRef.current = JSON.stringify(appDataRef.current);
        setSyncStatus('synced');
      } catch (error: any) {
        if (error?.state?.data) {
          sharedHydratingRef.current = true;
          sharedRevisionRef.current = Number(error.state.revision || sharedRevisionRef.current);
          lastSharedFingerprintRef.current = JSON.stringify(error.state.data);
          setAppData(error.state.data);
          sharedHydratingRef.current = false;
        }
  ...
  useEffect(() => {
    ...
    const poll = async () => {
      try {
        const remote = await fetchSharedState();
        if (!remote || remote.revision <= sharedRevisionRef.current) return;
        sharedHydratingRef.current = true;
        sharedRevisionRef.current = remote.revision;
        lastSharedFingerprintRef.current = JSON.stringify(remote.data);
        setAppData(remote.data);
  ...
  useEffect(() => {
    ...
    const unsubscribe = subscribeToUserTrips(
      ...
      async (firestoreTrips, metadata) => {
        ...
        const stampOf = (t: TripInfo) => String(t.serverUpdatedAt ?? t.updatedAt ?? t.createdAt ?? '');
        const nextStamps = Object.fromEntries(firestoreTrips.map((t) => [t.id, stampOf(t)]));
        ...
        const stampsUnchanged = firestoreTrips.length === Object.keys(previousStamps).length &&
          firestoreTrips.every((t) => previousStamps[t.id] === stampOf(t));
        ...
        // 4. Fetch the full bundle for the active trip (or default to the first
        //    available trip if this device has no active trip yet, e.g. fresh phone).
        const activeRemoteTrips = firestoreTrips.filter((t) => !deletedTripIds.has(t.id));
        let targetActiveId = appDataRef.current.activeTripId;
        if ((!targetActiveId || !activeRemoteTrips.some((t) => t.id === targetActiveId)) && activeRemoteTrips.length > 0) {
          targetActiveId = activeRemoteTrips[0].id;
        }
        const activeRemote = activeRemoteTrips.find((t) => t.id === targetActiveId);
        ...
        setAppData((prev) => {
          ...
          for (const tripInfo of activeRemoteTrips) {
            ...
            if (tripInfo.id === targetActiveId && activeCloudBundle) {
              ...
              // Brand new trip created on the other device or discovered on fresh install:
              // Initialize its bundle so it appears in the trip switcher and overview.
              nextTrips[tripInfo.id] = {
                tripInfo,
                itinerary: [],
                budget: [],
              ...
              rememberTripBaseline(tripInfo.id, baselineFromRemote(tripInfo, tripInfo.coverImage));
            }
          }
          ...
          const nextActiveId =
            (prev.activeTripId && nextTrips[prev.activeTripId])
              ? prev.activeTripId
              : (targetActiveId && nextTrips[targetActiveId])
                ? targetActiveId
  ...
  // One shared password for both phones. No email or Google account is required.
  const handleLoginSuccess = async (password: string) => {
    try {
      const ok = await loginSharedPassword(password);
      if (!ok) { showToast('Mật khẩu không đúng.', 'error'); return; }
      setSharedSession(true);
      setUserEmailState('Hai đứa');
      setAuthEmail('shared');
      setAppData((prev) => ({ ...prev, userEmail: 'shared' }));
      showToast('Đã mở sổ tay chung.', 'success');
    } catch (error: any) {
      showToast(error?.message === 'INVALID_PASSWORD' ? 'Mật khẩu không đúng.' : 'Không kết nối được máy chủ đồng bộ.', 'error');
    }
  };

  const handleConnectCloud = () => {
    if (sharedSession) showToast('Đã kết nối kho dữ liệu chung.', 'info');
  };

  const handleChangeSharedPassword = async (newPassword: string) => {
    try {
      await changeSharedPassword(newPassword);
      showToast('Đã đổi mật khẩu chung. Mật khẩu mới dùng cho cả hai máy.', 'success');
    } catch (error: any) {
      showToast(error?.message === 'PASSWORD_TOO_SHORT' ? 'Mật khẩu mới phải có ít nhất 6 ký tự.' : 'Đổi mật khẩu thất bại.', 'error');
      throw error;
    }
  };
  ...
  const handleSelectTrip = (tripId: string) => {
    if (appData.trips[tripId]) {
      ...
      if (firebaseUser) {
        fetchFullTripBundle(tripId, appData.trips[tripId].tripInfo)
          .then((fresh) => {
            if (!fresh) return;
            setAppData((prev) => {
              const current = prev.trips[tripId];
              if (!current) return prev;
              return {
                ...prev,
                trips: {
                  ...prev.trips,
                  [tripId]: mergeBundleWithCloud(current, fresh, getTripBaselines()[tripId])
                }
              };
            });
          })
          .catch((err) => console.warn('Background bundle fetch on select failed:', err));
      }
    }
  };
  ...
  return (
    <div id="travel-planner-app" className="min-h-screen bg-[#FAF7F2] text-[#3D312A] flex flex-col font-sans selection:bg-[#E2D2C3] selection:text-[#362417] pb-24 md:pb-6">
      ...
      <Navigation
        ...
        isConnectedToCloud={sharedSession}
